...
def ridgecv_incountry_wrapper(model_name: str, savedir: str) -> None:
    ...
    '''
    with mlflow.start_run(run_name=f"ridgecv_incountry_{model_name}"):
        # Log parameters
        mlflow.log_param("model_name", model_name)
        mlflow.log_param("savedir", savedir)
...
def ridgecv_incountry_concat_wrapper(model_names, savedir) -> None:
    ...
    - savedir: str, path to directory for saving ridge regression weights and predictions
    '''
    with mlflow.start_run(run_name="ridgecv_incountry_concat"):
        # Log parameters
        mlflow.log_param("model_names", model_names)
        mlflow.log_param("savedir", savedir)
