...
label = "income"
LOGS_ROOT_DIR = f'/home/sagemaker-user/reproducible/logs/{label}/'
DATASET_NAME = 'DHS_OOC_A'
base_path = f"/home/sagemaker-user/reproducible/logs/{label}"
MODEL_NAME_TO_DIR = {
...
#            'Ridge MS+NL hist': base_path + '/dhs_ridge/incountry_msnl_hist',
     ...
     # ResNet with Ridge Models
           'Resnet-18 MS - E': base_path + '/resnet_ms',
           # 'Resnet-18 MS - ADE':base_path + '/resnet_ms/3_folds',
           # 'Resnet-18 MS - All':base_path + '/resnet_ms/All_folds',
           #'Resnet-18 NL': base_path + '/resnet_nl',
           #'Resnet-18 MS_250': base_path + '/resnet_ms_250',
           #'Resnet-18 MS+NL concat': base_path + '/resnet_msnl_concat',
           #'Resnet-18 MS_250+NL concat': base_path + '/resnet_msnl_concat_250',
}
...
#npz = load_npz(f'/home/sagemaker-user/reproducible/Datasets/SelectedClusters/dhs_co_{label}.npz')
npz = load_npz(f'/home/sagemaker-user/reproducible/data/processed/dhs_co_{label}.npz')
...
def get_urban_rural_indices(locs, loc_dict):
    ...
    for i, loc in enumerate(locs):
        ...
        if loc not in loc_dict:
            ...
            rights.append(loc)
        if loc_dict[loc].get('urban') == 'RURAL':
        # !!
    ...
    return urban_indices, rural_indices, errors, rights

urban_indices, rural_indices, errors, rights = get_urban_rural_indices(locs, loc_dict)
...
urban_indices = []
rural_indices = np.arange(num_examples)


# Figure - Cluster Distribution Split by Cluster Type
# ![incountry_Urban_Rural.png](attachment:feb88cb5-fb19-4357-be84-7923d71252b2.png)
...
for model_name in MODEL_NAMES:
    if model_name in preds:
        if len(preds_df) != len(preds[model_name]):
            print(f'Modelo {model_name}', len(preds_df), ' <> ', len(preds[model_name]))
            continue
        print(len(preds[model_name]))
        
        preds_df[model_name] = preds[model_name]
    else:
        print('Não existe esse modelo: ', model_name)
...
def calculate_nan_inf_percentages(data):
    # Converte para float, ignorando erros em conversões não possíveis
    data = pd.to_numeric(data, errors='coerce')
    
    # Calcula a porcentagem de NaNs (inclui os que foram convertidos devido a erro)
    nan_percentage = data.isna().mean() * 100
    
    # Calcula a porcentagem de infinitos positivos e negativos
    inf_percentage = np.isinf(data).mean() * 100
    
    return nan_percentage, inf_percentage

# Aplica a função em cada coluna de preds_df e labels se necessário
nan_inf_percentages = {col: calculate_nan_inf_percentages(preds_df[col]) for col in preds_df.columns}

# Convertendo o dicionário para DataFrame para melhor visualização
nan_inf_df = pd.DataFrame(nan_inf_percentages, index=["NaN%", "Inf%"]).T
nan_inf_df
...
# Assuming the evaluation code executes here
overall_df = evaluate_df(preds_df, cols=MODEL_NAMES)


# Plot Ridge Evaluations 
...
# Tables with model Evaluation split for Urban/Rural Clusters
...
country_df.sort_values(by='r2', ascending=False, inplace=True)
...
country_df.to_csv('/home/sagemaker-user/reproducible/logs/income/country.csv')

country_urban_df.to_csv('/home/sagemaker-user/reproducible/logs/income/country_urban.csv')


# In[25]:


with mlflow.start_run(run_name='evaluate_models_income'):
    for index, row in country_df.reset_index().iterrows():
        # Log model
        mlflow.log_metric("model_name", row['model'])
...
# DataFrame with model evaluation split by Cluster Types
...
# Histogram with model evaluation (r2 or Squared pearson correlation coeficcient) split by Cluster Types
...
# Histogram with model evaluation (R2 or coefficient of determination) split by Cluster Types
...
# Plot Ridge Evaluations split by Cluster Type
...
nl0_df.to_csv('/home/sagemaker-user/reproducible/logs/income/performance_without_nl.csv')
with pd.option_context('display.max_rows', 20, 'display.precision', 5):
    display(nl0_df)
...
def plot_label_v_nl(label, preds, nl_means):
    '''
    Args
    - model_name: str
    - preds: np.array
...
# Plot the Target indicator in function of the Mean NL Value of the Image

# In[ ]:


plot_label_v_nl(label, labels, nls_mean)
plt.savefig("income_mean_NL")
...
# For each model, plots the predicted indicator in function of the Mean NL Value of the Image
