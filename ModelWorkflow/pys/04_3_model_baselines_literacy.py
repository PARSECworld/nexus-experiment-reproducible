...
LOGS_ROOT_DIR = f"/home/sagemaker-user/reproducible/logs/{LABEL_NAME}" 
print(LOGS_ROOT_DIR)
...
file_path =f"/home/sagemaker-user/reproducible/data/processed/dhs_co_{LABEL_NAME}.npz"
print(file_path)
...
with open(f'/home/sagemaker-user/reproducible/data/processed/loc_dict_brazil_2010_{LABEL_NAME}.pkl', 'rb') as f:
    loc_dict = pickle.load(f)

# Inicializar incountry_group_labels
incountry_group_labels = np.zeros_like(labels, dtype=np.int32)

# Ajustar os índices para garantir que todos estejam dentro do intervalo válido
...
# Construir o dicionário de índices por país
for i, loc in enumerate(locs):
    ...
    if loc_tuple in loc_dict:
        ...
        print(f"Localização {loc_tuple} não encontrada em loc_dict e será ignorada.")

# Definir valores dos países de interesse
values = ["brazil"]

# Atualizar country_labels com os índices dos países de interesse
for i, country in enumerate(values):
    if country in country_indices:
        ...
        print(f"Processado país: {country}")
    else:
        print(f"País {country} não encontrado em country_indices.")
        ...
        for nl_type, nls in [('center', nls_center), ('mean', nls_mean)]:
            for weighted, w in [(False, None), (True, households)]:
                ...
                try:
                    ...
                    print(f"Erro de divisão por zero para sensor {sensor}, nl_type {nl_type}, weighted {weighted}. Pulando...")
                    continue
...
def knn_cv_wrapper(labels, years, nls, folds, keep=1.0, seed=None, savepath=None, verbose=False):
    ...
    for i, f in enumerate(fold_keys):
        ...
        # Verificar e ajustar os índices de test_idxs
        ...
        for j, k in enumerate(ks):
            ...
            # Verificar e ajustar os índices de train_idxs e val_idxs
