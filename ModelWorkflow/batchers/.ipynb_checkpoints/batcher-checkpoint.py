...
from typing import Iterable, Optional, Mapping, Union, Tuple, Dict, List
...
class Batcher():
    ...
    def parse_tfrecord(self, example_proto):
        features = {
            ...
            # Inclua outras features conforme necessário
        }
        parsed_features = tf.io.parse_single_example(example_proto, features)
        # Continue o processamento conforme necessário
        return parsed_features
        
#     def get_batch_vtf2(self) -> tuple[tf.Operation, dict[str, tf.Tensor]]:
...
#         '''
#         # Carrega o dataset
#         dataset = tf.data.TFRecordDataset(self.tfrecord_files, compression_type='GZIP')

#         # Parse os registros TFRecord
#         dataset = dataset.map(self.parse_tfrecord, num_parallel_calls=tf.data.experimental.AUTOTUNE)

#         # Cache o dataset após o carregamento e parsing (opcional)
#         if self.cache:
#             dataset = dataset.cache()

#         # Embaralha os dados se necessário
#         if self.shuffle:
#             dataset = dataset.shuffle(buffer_size=1000)  # Ajuste buffer_size conforme necessário

#         # Repete o dataset para o número especificado de épocas
#         dataset = dataset.repeat(self.epochs)

#         # Agrupa os dados em batches
#         dataset = dataset.batch(self.batch_size)

#         # Prefetch: prepara os dados enquanto o modelo está sendo treinado
